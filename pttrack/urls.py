...
urlpatterns = [
    ...
    url(r'^clindate/(?P<pt_id>[0-9]+)/$', views.ClinicDateCreate.as_view(), name="new-clindate"),
    url(r'^(?P<pk>[0-9]+)/$', DetailView.as_view(model=mymodels.Patient), name='patient-detail'),
    url(r'^(?P<pt_id>[0-9]+)/followup/$', views.FollowupCreate.as_view(), name='new-followup'),
    url(r'^(?P<pt_id>[0-9]+)/workup/$', views.WorkupCreate.as_view(), name='new-workup'),
