...
class TestAttestations(TestCase):
    ...
    # Progress Note Attestation Testing
    def test_all_roles_provider_pn_signing(self):

        all_roles_provider = build_provider()  # roles=["AP", "C", "PV", "CV"], username=None, password='password', email=None)

        with self.assertRaises(ValueError):  # test that if a provider tries to sign a note with a provider type they do not have, an error is thrown
            self.pn.sign(
                all_roles_provider.associated_user,
                active_role=ProviderType(long_name="Test Provider", short_name="TP", signs_charts=False, staff_view=False))

        self.assertFalse(self.pn.signed())
        ...
        # test that provider given all roles can sign a note if their active role allows for it
        self.pn.sign(
            all_roles_provider.associated_user,
            active_role=ProviderType.objects.filter(signs_charts=True).first())

        self.assertTrue(self.pn.signed())

    def test_non_signing_provider_pn_signing(self):
    ...
    def test_one_role_provider_pn_signing(self):
        ...
        for role in ["Preclinical", "Clinical", "Coordinator", "Attending"]:  # ProviderType is not Iterable, even though the QuerySet should be iterable?
            ...
            if one_role_provider.clinical_roles.first().signs_charts:
                ...
                with self.assertRaises(ValueError):  # test that a non-Attending provider cannot use the Attending role to sign a note
                    self.pn.sign(
                        one_role_provider.associated_user,
                        active_role=ProviderType.objects.filter(signs_charts=True).first())
            ...
            self.pn.signer = None  # reset chart's signed status
