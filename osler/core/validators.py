...
def validate_zip(value):
    ...
    if len(str(value)) != 5:
        raise ValidationError(_('{0} is not a valid ZIP, because it has {1} digits.').format(str(value), len(str(value))))

    if not str(value).isdigit():
        raise ValidationError(
            _("%s is not a valid ZIP, because it contains non-digit characters.")
            % value)
...
def validate_birth_date(value, max_years=150):
    ...
    if today - value < datetime.timedelta(0):
        raise ValidationError(_("Birth dates cannot be in the future."))

    if today - value > datetime.timedelta(365 * max_years):
        raise ValidationError(
            _("Birth dates cannot be more than {0} years in the past.").format(
                max_years))
...
def validate_name(value):
    ...
    if value.startswith((' ', '\t')) or value.endswith((' ', '\t')):
        raise ValidationError(_("Name cannot start or end with a space"))
...
def validate_attending(value):
    ...
    if not attending.clinical_roles.filter(signs_charts=True).exists():
        raise ValidationError(_("This provider is not allowed to sign charts."))
